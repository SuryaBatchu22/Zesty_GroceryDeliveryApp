...
const Cart = () => {
    const { user,products, currency, cartItems, removeFromCart, getCartCount,
        updateCartItem, navigate, getCartAmount, axios , setCartItems} = useAppContext()
    ...
    return getCartCount() > 0 ? (
        ...
        <p  className='w-full flex mt-10 items-center justify-center text-lg font-medium'>No items in the cart</p>
        <button onClick={() => { navigate('/products'); scrollTo(0, 0); }} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
            <img src={assets.arrow_right_icon_colored} alt='arrow' className="group-hover:-translate-x-1 transition" />
            Continue Shopping
        </button></div>
